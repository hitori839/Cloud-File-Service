...
> **현재 구현 기준:** 이 문서는 수동 ECS 배포 흐름과 Terraform 배포 흐름을 모두 설명한다. 현재 `infra/ecs-task-definition.json`과 `infra/terraform/ecs.tf`는 리소스 이름, DB 설정, 네트워크 구성이 다를 수 있으므로 한 경로만 선택해 사용한다. 실제 계정 ID, ARN, ECR 주소, DB endpoint와 비밀번호는 문서에 기록하지 않고 placeholder 또는 실행 환경 변수로 제공한다.

> **보안 주의:** ECS task definition과 로그에 DB 비밀번호나 AWS secret을 평문으로 넣지 않는다. 운영 환경에서는 Secrets Manager, SSM Parameter Store, ECS task role과 execution role을 사용한다.
...
```json
{
  ...
  "Arn": "YOUR_EXECUTION_IDENTITY_ARN"
}
```
...
```text
YOUR_AWS_ACCOUNT_ID.dkr.ecr.ap-northeast-2.amazonaws.com/cloud-file-service
```
